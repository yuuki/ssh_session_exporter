...
func TestParseLine(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "accepted publickey sshd-session (OpenSSH 9.8+)",
			line:     `Sep  2 05:06:41 localhost sshd-session[15424]: Accepted publickey for probe from 192.168.5.2 port 59614 ssh2`,
			wantType: EventAuthSuccess,
			wantUser: "probe",
			wantIP:   "192.168.5.2",
			wantMeth: "publickey",
		...
		{
			name:     "failed password sshd-auth (OpenSSH 10.0+)",
			line:     `Jan  1 12:00:00 server sshd-auth[8123]: Failed password for alice from 192.168.1.10 port 22 ssh2`,
			wantType: EventAuthFailure,
			wantUser: "alice",
			wantIP:   "192.168.1.10",
			wantMeth: "password",
		...
		{
			name:     "invalid user sshd-session",
			line:     `Jan  1 12:00:00 server sshd-session[3003]: Invalid user admin from 10.0.0.5 port 22`,
		...
		{
			name:     "preauth disconnected sshd-session",
			line:     `Jan  1 12:00:00 server sshd-session[4003]: Disconnected from authenticating user root 10.0.0.1 port 22 [preauth]`,
